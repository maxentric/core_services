...
FILE_CONTENTS = """# OLSR.org routing daemon config file
...
IpcConnect
{
     ...
     #MaxConnections  2	# SG: Changed from Default (0)
...
LoadPlugin "/usr/local/lib/olsrd_jsoninfo.so.1.1" {
}
...
Interface {InterfaceName}
{
    ...
    Ip4Broadcast	255.255.255.255 # SG: Added
    ...
    # HelloInterval       2.0
    HelloValidityTime   6.0	# SG: Modified to 3 times Hello interval
    # TcInterval          5.0
    TcValidityTime      15.0	# SG: Modified to 3 times TC interval
    # MidInterval         5.0
    MidValidityTime     15.0	# SG: Modified to 3 times MID interval
    # HnaInterval         5.0
    HnaValidityTime     15.0	# SG: Modified to 3 times HNA interval
...
class OLSRdService(CoreService):
    """
    Example Custom CORE Service - OLSR
    ...
    name: str = "OLSRdService"
    group: str = "maXentric"
    executables: Tuple[str, ...] = ()
    dependencies: Tuple[str, ...] = ()
    #dirs: Tuple[str, ...] = ()
    dirs: list[str] = [] #["/usr/local/core1",]
    configs: Tuple[str, ...] = ("myOlsrd.conf","myOlsrd.sh")
    startup: Tuple[str, ...] = (f"chmod +x {configs[1]}", f"./{configs[1]}")
    validate: Tuple[str, ...] = ()
    ...
    def generate_config(cls, node: CoreNode, filename: str) -> str:
        ...
        """
        if filename == cls.configs[0]:  # Config file
            # Create the customized config file
            #with open(OLSR_CONFIG_FILENAME) as ff:
                cfg = f'# Customized OLSRd configure file for node {node.name}\n'
                
                #for line in ff:
                for line in list(iter(FILE_CONTENTS.splitlines())):
                    
                    # Update the IP address
                    if "{IpAddress}" in line:
                        line = line.replace("{IpAddress}", str((node.get_ifaces()[0]).get_ip4 ().ip))
                    
                    # Update number of interfaces
                    elif " {InterfaceName}" in line:
                        allIfaces = ""
                        for iface in node.get_ifaces():
                            if "eth" in iface.name:
                                allIfaces += " \"" + str(iface.name) + "\""
                        line = line.replace(" {InterfaceName}", allIfaces)
                    
                    cfg += line+"\n"

        elif filename == cls.configs[1]:    # Script file
            cfg = "#!/bin/sh\n"
            cfg += f'olsrd -f {cls.configs[0]}'
        return cfg
